...
#if defined(NDEBUG)
# error "Billionrichercoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Billionrichercoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Billionrichercoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Billionrichercoin: find block wanted by given orphan block
...
// Billionrichercoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Billionrichercoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Billionrichercoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Billionrichercoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Billionrichercoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Billionrichercoin: track money supply and mint amount info
...
// Billionrichercoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Billionrichercoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Billionrichercoin: compute stake entropy bit for stake modifier
    ...
    // Billionrichercoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Billionrichercoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Billionrichercoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Billionrichercoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Billionrichercoin-loadblk");
